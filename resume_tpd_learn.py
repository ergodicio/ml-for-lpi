...
def train_model(_cfg_path, td, epoch=0):
    ...
    if epoch:
        opt_state_path = os.path.join(td, "opt-state.pkl")
        s3.download_file(bucket_name, f"188371/{run_id}/artifacts/opt-state-epoch={epoch}-batch-0.yaml", opt_state_path)
        with open(opt_state_path, "rb") as fi:
            opt_state = pickle.load(fi)

        diff_params_path = f"s3://public-ergodic-continuum/188371/{run_id}/artifacts/weights-e{epoch:02d}-b00.eqx"
        cfg["drivers"]["E0"]["file"] = diff_params_path
        orig_cfg["drivers"]["E0"]["file"] = diff_params_path
        cfg["mlflow"]["run"] = f"resume-{run_id}-epoch-{epoch}"
    ...
    intensities = np.linspace(1e14, 1e15, 5)
    ...
    with mlflow.start_run(run_id=exo.mlflow_run_id, log_system_metrics=True) as mlflow_run:
        ...
        with parsl.load(parsl_config):
            for i in range(epoch, 200):  # 1000 epochs
                ...
                for j in range(num_batches):
                    ...
                    training_data = all_training_data[j * cfg["opt"]["batch_size"] : (j + 1) * cfg["opt"]["batch_size"]]
                ...
                mlflow.log_metrics(
                    {"epoch loss": float(np.mean(epoch_loss)), "epoch grad norm": float(np.mean(epoch_gradnorm))},
                    step=i,
                )


if __name__ == "__main__":
    import argparse, boto3, tempfile
    ...
    with tempfile.TemporaryDirectory(dir=BASE_TEMPDIR) as td:
        if args.run_id is not None:
            run_id = args.run_id
            s3 = boto3.client("s3")
            cfg_path = os.path.join(BASE_TEMPDIR, "config.yaml")
            bucket_name = "public-ergodic-continuum"
            file_path = f"188371/{run_id}/artifacts/config.yaml"
            s3.download_file(bucket_name, file_path, cfg_path)
        else:
            cfg_path = args.config

        train_model(cfg_path, td, epoch=1)
